...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=SUPPRESS,
        help="The directory of the project to update version specifiers for.",
    )
    parser.add_argument(
        "--support-days",
        type=int,
    ...
    support_days = datetime.timedelta(days=support_days)
    current_date = datetime.datetime.now(tz=datetime.timezone.utc)

    # Read the package metadata
    pyproject = read_pyproject(project_root=project_root, to_dict=False)
    extended_metadata = read_extended_metadata(
        os.path.join(project_root, ".extended_metadata.yaml")
    )
    bump_packages = extended_metadata.get("spec_zero_update_packages", [])
    if len(bump_packages) == 0:
        raise RuntimeError(
            "No packages specified under `spec_zero_update_packages` of "
            "`.extended_metadata.yaml`. Nothing to update."
        )

    # Get dependencies from pyproject.toml
    core_dependencies = pyproject["project"]["dependencies"]
    opt_dependencies = pyproject["project"].get("optional-dependencies", {})
    dep_groups = pyproject.get("dependency-groups", {})

    # Find release and drop dates for desired packages
    package_releases = {
        package: _get_release_and_drop_dates(package, support_days, current_date)
        for package in bump_packages
    }

    # Update version specifiers in-place
    changed = []
    changed = _update_specifiers(
        core_dependencies, package_releases, support_days, changed=changed, label="Core"
    )
    for opt_deps in opt_dependencies.values():
        changed = _update_specifiers(
            opt_deps, package_releases, support_days, changed=changed, label="Optional"
        )
    for opt_deps in dep_groups.values():  # don't add dev deps to changelog
        _update_specifiers(opt_deps, package_releases, support_days)
...
def _get_release_and_drop_dates(
    package: str, support_days: datetime.timedelta, current_date: datetime.datetime
) -> dict:
    """Get release and drop dates for a given package from pypi.org."""
    releases = {}
    logger.info("Querying pypi.org for %s versions...", package)
    ...
    for idx, dep in enumerate(dependencies):
        ...
        if pkg_name in releases:  # check if this is a package to update
            ...
            if len(pre_cutoff_matches) == 0:
                raise RuntimeError(
                    f"{pkg_name} had no versions available {support_days.days} "
                    "days ago compliant with the following specifier(s): "
            ...
            for ver in post_cutoff_matches:
                if as_minor_version(ver) > min_ver:  # if a new minor version
            ...
            min_ver_spec = SpecifierSet(f">={min_ver!s}")
...
def _find_version_to_pin_and_release(
    version: Version, specifier: SpecifierSet, all_versions: list, release_dates: dict
) -> tuple[Version, datetime.datetime]:
    ...
    version = min(
        ...
        if as_minor_version(ver) == as_minor_version(version)
    )
    # Check unpatched form of this version is not excluded by existing specifiers
    use_patch = not specifier.contains(as_minor_version(version))
...
def _prettify_requirement(req: Requirement) -> str:
